...
const Features = () => {
  return (
    <section
      id='features'
      className='w-full bg-muted py-12 flex flex-col items-center justify-center'
    >
      <div className='container px-6 xs:px-8 sm:px-0 sm:mx-8 lg:mx-auto grid grid-cols-1 lg:grid-cols-2 gap-8 relative bg-muted'>
        <div className='col-span-full space-y-4'>
          <AnimatedGradientText className='ml-0 bg-background backdrop-blur-0'>
            <span
              className={cn(
                'inline animate-gradient bg-gradient-to-r from-[#ffaa40] via-[#9c40ff] bg-[length:var(--bg-size)_100%] bg-clip-text text-transparent'
              )}
            >
              Features
            </span>
        ...
        <div className='flex flex-col justify-start items-start order-2 lg:order-1'>
          {featureList.map(feature => {
            return (
              <div
                key={feature.title}
                className='flex items-start gap-2 sm:gap-4 rounded-lg my-8 lg:p-12'
              >
